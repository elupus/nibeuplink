...
async def uplink_fixture(aioresp: aioresponses):
    async with UplinkSession(MOCK_CLIENT_ID, MOCK_CLIENT_SECRET, MOCK_REDIRECT_URI) as session:
        uplink = Uplink(session)
        yield uplink

async def test_get_system(aioresp: aioresponses, uplink: Uplink):
    aioresp.add(f"https://api.nibeuplink.com/api/v1/systems/{MOCK_SYSTEMID}", method="GET", payload=MOCK_SYSTEM_1)
...
async def test_post_smarthome_thermostats(aioresp: aioresponses, uplink: Uplink):
    ...
    aioresp.add(f"https://api.nibeuplink.com/api/v1/systems/{MOCK_SYSTEMID}/smarthome/thermostats", method="POST", callback=_callback)

    thermostat = SetThermostatModel(externalId=1, name="name1", actualTemp=30, targetTemp=None, valuePosition=None, climateSystems=[1])

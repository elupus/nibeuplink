...
class Uplink:
    def __init__(self, loop):
        ...
        self.app.router.add_routes(
            [
                ...
                web.get(
                    "/api/v1/systems/{systemId}/notifications", self.on_notifications
                ),
                web.get(
                    "/api/v1/systems/{systemId}/parameters", self.on_get_parameters
                ),
                web.put(
                    "/api/v1/systems/{systemId}/parameters", self.on_put_parameters
                ),
            ]
        )
        self.runner = None
    ...
    async def on_oauth_token(self, request):
        ...
        if data["grant_type"] == "authorization_code":
            ...
            raise JsonError(
                400,
                "invalid_request",
                "unknown grant_type: {}".format(data["grant_type"]),
            )
    ...
    async def on_oauth_authorize(self, request):
        ...
        url[3] = urlencode({"state": query["state"], "code": "dummycode",})
    ...
    def add_notification(self, systemid, notification):
        self.systems[systemid].notifications[
